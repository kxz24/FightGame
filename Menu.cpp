...
void Menu::drawMainMenu(int frame) {
    ...
    // 加载背景图片
    IMAGE background;
    loadimage(&background, _T("rec/background1.png"), 1200, 600, 1); 
    ...
    settextcolor(color1);
    outtextxy(x + dx1, y + dy1, _T("Crystal Fighter"));

    // 中间层标题
    settextstyle(76, 52, _T("Arial Black"));
    settextcolor(color2);
    outtextxy(x + dx2, y + dy2, _T("Crystal Fighter"));

    // 顶层标题
    settextstyle(72, 50, _T("Arial Black"));
    settextcolor(color3);
    outtextxy(x + dx3, y + dy3, _T("Crystal Fighter"));
